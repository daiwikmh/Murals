import type { WalletClient, PublicClient, Hash, TransactionReceipt, Address, Chain, BlockTag, Account, WaitForTransactionReceiptParameters } from 'viem';
...
export type PerformTransactionGasLimit = (overrides: TransactionOptions) => Promise<bigint>;
export type PerformTransactionSendTransaction = (override: TransactionOptions) => Promise<Hash>;
export type PerformTransactionDecodeResult<TDecodedResult> = (receipt: TransactionReceipt) => Promise<TDecodedResult>;
type PerformTransactionOptionsDecodePartial<TDecodedResult> = TDecodedResult extends undefined ? {
    decodeResult?: undefined;
} : {
    decodeResult: PerformTransactionDecodeResult<TDecodedResult>;
};
export type PerformTransactionOptions<TDecodedResult> = CommonTransactionProps & {
...
export type NoTxOptions<TProps extends CommonTransactionProps> = Omit<TProps, 'callback' | 'waitForTransactionReceiptParameters'>;
...
type TransactionCallbackReturn<TProps> = TProps extends {
    stage: TransactionCallbackStage.SIGN;
} ? bigint | undefined : void;
export type TransactionCallbackResult<TProps> = TransactionCallbackReturn<TProps> | Promise<TransactionCallbackReturn<TProps>>;
export type TransactionCallback = (props: TransactionCallbackProps) => TransactionCallbackResult<TransactionCallbackProps>;
